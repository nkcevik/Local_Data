...
class Similarity:
    ...
    def __init__(self, similarity_algorithm, accuracy_ratio, column, unique_column, dataset, file_name):
    ...
    def clean_dataframe(self):
        ...
        df = df[[self.column, self.grup_column, self.score_column]]        
    ...
    def all_similarity_algorithm(self):
        ...
        file = "/data/cikti_all_" + self.file_name  + ".xlsx" 
        path = path + file
        
        algorithm_grup_number = [0, 0, 0]
        algorithm_grup = ["GRUP_LEV", "GRUP_COS", "GRUP_JAC"]
        algorithm_score = ["SCORE_LEV", "SCORE_COS", "SCORE_JAC"]
        for index in range(0, 3):
           # df[algorithm_grup[index]].iloc[0] = algorithm_grup_number[index]
           # df[algorithm_score[index]].iloc[0] =  "base"
...
# dataset["DATA"] = dataset["MANDT"].map(str) + dataset["NAME1"].map(str) 
# dataset = pd.DataFrame(dataset["DATA"], index = dataset["KAYIT SIRASI"])
# dataset = dataset[["DATA", "KAYIT SIRASI"]]
...
# ts = Similarity("all", 80, "DATA", "KAYIT SIRASI", dataset)
# print(ts.column)
