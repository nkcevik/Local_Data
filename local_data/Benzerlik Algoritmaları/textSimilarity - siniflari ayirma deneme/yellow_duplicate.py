...
path = os.path.dirname(__file__) + "/cikti.xlsx"
df = pd.read_excel(path)
df['CNTRL'] = pd.Series(np.zeros(len(df)), index=df.index)

count = 0
# list_of_deleted_index = np.array()
...
for index1 in range(2, len(df)):
        ...
        for index2 in range(3, len(df)):
                ...
                if df["GRUP"].iloc[index1] == df["GRUP"].iloc[index2]:
                        # print("grup-index1: ", df["GRUP"].iloc[index1])
                        # print("grup-index2: ", df["GRUP"].iloc[index2])
                        # print("----------------------------------")
        ...
        #        count += 1
                # print("I'm also here.")
        #        df["GRUP"].iloc[index1] = "-"
        #        print("grup: ", count," - ", df["GRUP"].iloc[index1])
                # list_of_deleted_index = list_of_deleted_index.append(df[list_index])
...
path = os.path.dirname(__file__) + "/cikti_grup.xlsx"
print("xx")
df.to_excel(path)
