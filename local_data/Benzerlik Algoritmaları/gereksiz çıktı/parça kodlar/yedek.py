...
class Similarity:
                    ...
                    if index1 != index2:
                        ...
                        if sc > self.accuracy_ratio:
                            if df[algorithm_grup[index]].iloc[index2] == 0:
                                ...
                                df[algorithm_score[index]].iloc[index2] = sc       
            ...
            for index2 in range(100):
                if index1 != index2:
                    ...
                    if score > self.accuracy_ratio:
                        #print(score)
                        #print(df[self.grup_column].iloc[index2])
                        if df[self.grup_column].iloc[index2] == 0:
                            df[self.grup_column].iloc[index2] = grup
                            df[self.score_column].iloc[index2] = score
                   # else:
                   #     if df[self.score_column].iloc[index2] < score and df[self.grup_column].iloc[index2] != -1:
                   #         df[self.grup_column].iloc[index2] = grup
                   #         df[self.score_column].iloc[index2] = score
                   #         
    ...
    def scores(self, x, y):
        ...
        # cosin 
            ...
            score = self.get_cosine(x, y) * 100

        # jaccard
        elif self.similarity_algorithm == "jaccard":
            score = self.jaccard_similarity(x, y)

        elif self.similarity_algorithm == "all":
            score = self.jaccard_similarity(x, y)
            #score = self.all_similarity(x, y)
...
# dataset = pd.read_excel(r'ornek_malzeme_verisi_active.xlsx')
...
# example1
ts = Similarity("all", 60, "MLZ_KISA_TANIM_TR", "MLZNO", dataset)
# print(ts.column)
